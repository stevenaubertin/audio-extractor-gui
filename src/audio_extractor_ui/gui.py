...
from .utils import validate_file_path, validate_url, is_video_file
...
class AudioExtractorGUI:
    ...
    def setup_file_tab(self, parent):
        ...
        # File selection
        ttk.Label(parent, text="Select Video File:").pack(anchor='w', pady=(10, 5))
        
        file_frame = ttk.Frame(parent)
        file_frame.pack(fill='x', pady=(0, 10))
        
        self.file_path_var = tk.StringVar()
        ttk.Entry(file_frame, textvariable=self.file_path_var).pack(side='left', fill='x', expand=True, padx=(0, 5))
        ...
        format_combo = ttk.Combobox(parent, textvariable=self.format_var, values=self.extractor.get_supported_formats())
        format_combo.pack(fill='x', pady=(0, 10))
        
        # Quality selection
        ttk.Label(parent, text="Quality:").pack(anchor='w', pady=(10, 5))
        self.quality_var = tk.StringVar(value="high")
        quality_combo = ttk.Combobox(parent, textvariable=self.quality_var, values=self.extractor.get_quality_options())
        quality_combo.pack(fill='x', pady=(0, 10))
        
        # Extract button
        ttk.Button(parent, text="Extract Audio", command=self.extract_from_file).pack(pady=20)
    ...
    def setup_url_tab(self, parent):
        ...
        # URL input
        ttk.Label(parent, text="Enter Video URL:").pack(anchor='w', pady=(10, 5))
        
        self.url_var = tk.StringVar()
        ttk.Entry(parent, textvariable=self.url_var).pack(fill='x', pady=(0, 10))
        
        # Format selection
        ttk.Label(parent, text="Output Format:").pack(anchor='w', pady=(10, 5))
        self.url_format_var = tk.StringVar(value="mp3")
        format_combo = ttk.Combobox(parent, textvariable=self.url_format_var, values=self.extractor.get_supported_formats())
        format_combo.pack(fill='x', pady=(0, 10))
        
        # Quality selection
        ttk.Label(parent, text="Quality:").pack(anchor='w', pady=(10, 5))
        self.url_quality_var = tk.StringVar(value="high")
        quality_combo = ttk.Combobox(parent, textvariable=self.url_quality_var, values=self.extractor.get_quality_options())
        quality_combo.pack(fill='x', pady=(0, 10))
        
        # Extract button
        ttk.Button(parent, text="Extract Audio", command=self.extract_from_url).pack(pady=20)
    ...
    def extract_from_file(self):
        ...
        if not is_video_file(file_path):
            if not messagebox.askyesno("Warning", "Selected file may not be a video file. Continue anyway?"):
                return
        ...
        try:
            success = self.extractor.extract_from_file(
                file_path,
                self.format_var.get(),
    ...
    def extract_from_url(self):
        ...
        if not validate_url(url):
            if not messagebox.askyesno("Warning", "URL format may be invalid. Continue anyway?"):
                return
        ...
        try:
            success = self.extractor.extract_from_url(
                url,
